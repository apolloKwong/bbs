...
public class ReplyResource extends BaseResource<Reply> {
	...
	@GET
    @Path(value = "/getByTargetUserId/{targetUserId}")
    @Operation(code = Operation.READ, desc = Operation.READ_DESC)
    public List<Reply> getByTargetUserId(@PathParam("targetUserId")long targetUserId) {
      
      return getService().getByTargetUserId(targetUserId);
    }
	@GET
