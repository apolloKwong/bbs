...
const List = ({ onDeleteItem, onEditItem, isMotion, location,dispatch, ...tableProps }) => {
  ...
  const columns = [
	 {
		...
		render(text,record) {
            return <span><Link to={`../workflow/inst/${record.id}`}>{text}</Link></span>; 
		}
	 },{
		...
		title: i18n('lab.wf.handler'),
		dataIndex: 'handler',
		render(value , text) {
			if(value){
				return <Handler handler={value}></Handler>
			}
			return value;
		}
	 },{
  ...
  return (
    <div>
    	<Grid  {...tableProps} rowKey='id' columns={columns} title={i18n('lab.wf.handinlist')} />
    </div>
  )
}
